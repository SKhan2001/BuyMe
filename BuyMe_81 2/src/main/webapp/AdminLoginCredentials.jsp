...
<% 	
	try {
		...
		ResultSet rs = stmt.executeQuery("select Username, Password from Accounts where Username = '" + name + "'and Password = '" + pass + "' and isAdmin is true and isCustomerRep is not true;");
		session.setAttribute("name", name);
		session.setAttribute("pass", pass);
		if (rs.next()) {
				response.sendRedirect("WelcomeAdmin.jsp?name=" + name + "");
		} else {
